// Full optimized and user-friendly MCX Trading App with smooth auto-download, UI updates, and alert dialog integration
...
import { MCXApiService, type OptionChainData } from "../services/mcx-api"
...
export default function MCXTradingApp() {
  ...
  const [lastAutoDownloadInfo, setLastAutoDownloadInfo] = useState<{ count: number; timestamp: Date } | null>(null)
  ...
  const checkForAlerts = (oldData: OptionChainData[], newData: OptionChainData[]) => {
    ...
    newData.forEach((newItem, index) => {
      ...
      if (peChangePercent >= 2) {
        triggeredAlerts.push(`PE price changed by ${peChangePercent.toFixed(2)}% at strike ${newItem.STRIKE_PRICE}`)
      }

      if (ceChangePercent >= 2) {
        triggeredAlerts.push(`CE price changed by ${ceChangePercent.toFixed(2)}% at strike ${newItem.STRIKE_PRICE}`)
      }
    })

    if (triggeredAlerts.length > 0) {
      setAlerts((prev) => [...triggeredAlerts, ...prev].slice(0, 10))
    }
  }
  ...
  const performAutoDownload = useCallback(async (currentRecordCount: number) => {
    ...
    const csvContent = [
      headers.join(","),
      ...historicalData.map((row) => ["CRUDEOIL", "17JUL2025", row.STRIKE_PRICE, row.PE_LTP, row.CE_LTP].join(",")),
    ].join("\n")
  ...
  toast(
  <div>
    <div className="font-bold">📊 MCX Data Export Complete</div>
    <div>{`${currentRecordCount} historical records exported in MCX format`}</div>
  </div>,
  { duration: 3000 }
)
  ...
  const fetchOptionChainData = useCallback(async () => {
    ...
    try {
      ...
      if (isDataChanged(optionChainData, data)) {
        ...
        checkForAlerts(optionChainData, data)

        setOptionChainData(data)

        setHistoricalData((prev) => {
          const updatedHistoricalData = [...prev, ...data].slice(-1000)
      ...
      if (isClient) {
        toast(
          <div>
            ...
            <div>Failed to fetch option chain data.</div>
          </div>,
          { duration: 3000 }
        )
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchOptionChainData, 10000)
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-full mx-auto space-y-6">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={() => performAutoDownload(historicalData.length)} className="flex items-center gap-2">
        ...
        {alerts.length > 0 && (
          <Alert className="bg-yellow-100">
            ...
            <AlertDescription>
              {alerts[0]} <span className="text-xs text-gray-500 ml-2">(Most recent alert)</span>
            </AlertDescription>
          </Alert>
        )}
              ...
              <div className="flex items-center gap-2">
                {autoDownloadSettings.enabled && (
                  <Badge variant="outline" className="text-xs">
                    Auto-download: {autoDownloadSettings.recordThreshold - historicalData.length > 0
                      ? `${autoDownloadSettings.recordThreshold - historicalData.length} records remaining`
                      : "Ready to download"}
                  </Badge>
                )}
                <Badge variant={historicalData.length >= autoDownloadSettings.recordThreshold ? "destructive" : "secondary"}>
          ...
          <CardContent>
            ...
            <div className="mt-4">
              ...
              <p className="text-xs text-gray-500 mt-1">
                ...
                {autoDownloadSettings.enabled && historicalData.length >= autoDownloadSettings.recordThreshold &&
